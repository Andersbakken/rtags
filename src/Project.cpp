    ...
    if (!Server::instance()->suspended()) {
        ...
        if (mIndexParseData.compileCommandsFileId) {
            ...
            if (!lastModified) {
                ...
                const bool changed = (mode == Check_Explicit) || lastModified != mIndexParseData.lastModifiedMs;
                if (changed && Server::instance()->loadCompileCommands(data, file, mIndexParseData.environment, &cache)) {
                    found = true;
                }
            }
        }
