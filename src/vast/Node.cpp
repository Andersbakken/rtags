...
Node::Node(Node *parent, NodeType nodeType, const CXCursor &cursor)
    : mNodeType(nodeType)
    , mParent(parent)
    , mFlags(parent->mFlags & NodeFlagsMask)
    , mClangCursor(cursor)
    , mClangType(clang_getNullType())
{
...
Node::Node(Node *parent, NodeType nodeType, const CXType &type)
    ...
    , mParent(parent)
    , mFlags(parent->mFlags & NodeFlagsMask)
    , mClangCursor(clang_getNullCursor())
    , mClangType(type)
{
                    ...
                    std::function<void()> *init)
{
    ...
    switch (nodeType) {
        ...
        case Children:
            *vector = &mChildren;
            initializer(init, &Node::children);
            break;
        case Argument:
            *vector = &mArguments;
            initializer(init, &Node::arguments);
            break;
        case ArgumentType:
            *vector = &mArgumentTypes;
            initializer(init, &Node::argumentTypes);
            break;
        case OverloadedDeclaration:
            *vector = &mOverloadedDecls;
            initializer(init, &Node::overloadedDecls);
            break;
        case TemplateArgument:
            *vector = &mTemplateArguments;
            initializer(init, &Node::templateArguments);
            break;
        case Reference:
            *variable = &mReference;
            initializer(init, &Node::reference);
            break;
        case SemanticParent:
            *variable = &mSemanticParent;
            initializer(init, &Node::semanticParent);
            break;
        case LexicalParent:
            *variable = &mLexicalParent;
            initializer(init, &Node::lexicalParent);
            break;
        case Definition:
            *variable = &mDefinition;
            initializer(init, &Node::definition);
            break;
        case Canonical:
            *variable = &mCanonical;
            initializer(init, &Node::canonical);
            break;
        case SpecializedCursorTemplate:
            *variable = &mSpecializedCursorTemplate;
            initializer(init, &Node::specializedCursorTemplate);
            break;
        case TypedefUnderlyingType:
            *variable = &mTypedefUnderlyingType;
            initializer(init, &Node::typedefUnderlyingType);
            break;
        case EnumIntegerType:
            *variable = &mEnumIntegerType;
            initializer(init, &Node::enumIntegerType);
            break;
        case CanonicalType:
            *variable = &mCanonicalType;
            initializer(init, &Node::canonicalType);
            break;
        case PointeeType:
            *variable = &mPointeeType;
            initializer(init, &Node::pointeeType);
            break;
        case Result:
            *variable = &mResult;
            initializer(init, &Node::resultType);
            break;
        case ElementType:
            *variable = &mElementType;
            initializer(init, &Node::elementType);
            break;
        case TypeDeclaration:
            *variable = &mTypeDeclaration;
            initializer(init, &Node::typeDeclaration);
            break;
        case Type:
            *variable = &mType;
            initializer(init, &Node::type);
            break;
    }
}
