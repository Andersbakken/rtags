...
(defun rtags-enable-standard-keybindings (&optional map prefix)
  ...
  (unless prefix
    (setq prefix "C-x r"))
  (let ((k (lambda (key) (kbd (concat prefix " " key)))))
    (define-key map (funcall k ".") (function rtags-find-symbol-at-point))
    (define-key map (funcall k ",") (function rtags-find-references-at-point))
    (define-key map (funcall k "v") (function rtags-find-virtuals-at-point))
    (define-key map (funcall k "V") (function rtags-print-enum-value-at-point))
    (define-key map (funcall k "/") (function rtags-find-all-references-at-point))
    (define-key map (funcall k "Y") (function rtags-cycle-overlays-on-screen))
    (define-key map (funcall k ">") (function rtags-find-symbol))
    (define-key map (funcall k "<") (function rtags-find-references))
    (define-key map (funcall k "[") (function rtags-location-stack-back))
    (define-key map (funcall k "]") (function rtags-location-stack-forward))
    (define-key map (funcall k "C") (function rtags-switch-to-completion-buffer))
    (define-key map (funcall k "D") (function rtags-diagnostics))
    (define-key map (funcall k "G") (function rtags-guess-function-at-point))
    (define-key map (funcall k "p") (function rtags-set-current-project))
    (define-key map (funcall k "P") (function rtags-print-dependencies))
    (define-key map (funcall k "e") (function rtags-reparse-file))
    (define-key map (funcall k "E") (function rtags-preprocess-file))
    (define-key map (funcall k "R") (function rtags-rename-symbol))
    (define-key map (funcall k "U") (function rtags-print-cursorinfo))
    (define-key map (funcall k "O") (function rtags-goto-offset))
    (define-key map (funcall k ";") (function rtags-find-file))
    (define-key map (funcall k "F") (function rtags-fixit))
    (define-key map (funcall k "x") (function rtags-fix-fixit-at-point))
    (define-key map (funcall k "B") (function rtags-show-rtags-buffer))
    (define-key map (funcall k "I") (function rtags-imenu))
    (define-key map (funcall k "T") (function rtags-taglist)))
  )
