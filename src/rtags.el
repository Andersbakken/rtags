    ...
    (if fn
        (let ((preprocess-buffer (rtags-get-buffer (format "*RTags preprocessed %s*" fn))))
          ...
          (with-current-buffer preprocess-buffer
            (rtags-call-rc :path fn "--preprocess" fn)
            ...
            ((eq (car doc) 'progress)
             ...
             (while body
               (cond ((eq (caar body) 'index)
                      ...
                      (setq rtags-last-index (string-to-int (cdar body))))
                     ((eq (caar body) 'total)
                      (setq rtags-last-total (string-to-int (cdar body))))
                     (t error "Unexpected element %s" (caar body)))
               (setq body (cdr body)))
             (force-mode-line-update))
            ;;             (message "RTags: %s/%s (%s%%)" index total)))
            (t (error "Unexpected root element %s" (car doc)))
            )
      )
    )
...
;;   (let ((lint (string-to-int l))
;;         (rint (string-to-int r)))
;;     (cond ((< lint rint) -1)
;;           ((> lint rint) 1)
;;           (t 0))
;;     )
      ...
      (while lines
        (let ((cur (car lines))
          ...
          (when (string-match "\\(.*\\):\\([0-9]+\\):\\([0-9]+\\)" cur)
            ...
            (setq offset (rtags-offset-for-line-column (string-to-int (match-string 2 cur))
