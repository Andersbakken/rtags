...
enum { Format = 0x1 };
QByteArray GccArguments::Data::save() const
{
    QByteArray ret;
    // QDataStream ds(&ret, QIODevice::WriteOnly);
    // ds << static_cast<quint8>(Format)
    //    << input << output << x << c << args << error << static_cast<quint8>(language)
    //    << inputreplace << outputreplace << raw << dir;
    return ret;
}

bool GccArguments::Data::load(const QByteArray &data)
{
    if (data.isEmpty())
        return false;
    // QDataStream ds(data);
    ...
    // if (u != Format)
    //     return false;
    // ds >> input >> output >> x >> c >> args >> error
    //    >> u >> inputreplace >> outputreplace >> raw >> dir;
    // language = static_cast<Language>(u);
    return true;
}

QDataStream &GccArguments::operator>>(QDataStream &ds)
{
    // m_ptr.detach();
    // m_ptr->load(
    // m_ptr->
...
QDataStream &GccArguments::operator<<(QDataStream &ds) const
{
