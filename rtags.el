...
(defun rtags-next-prev-match (next)
  (if (get-buffer rtags-buffer-name)
      (let (target
        ...
        (cond ((and (= (point-at-bol) (point-min)) (not next)) (setq target (point-max)))
              ((and (= (point-at-eol) (point-max)) next) (setq target (point-min)))
              (next (setq target (point-at-bol 2)))
              (t (setq target (point-at-bol -1))))
        (goto-char target)
        (beginning-of-line)
